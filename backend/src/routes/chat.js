...
import { checkGuardrails } from "../chat/guardrails.js"; 
...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    // --- GUARDRAIL CHECK ---
    // If the AI returned a refusal message instead of a SELECT query, 
    // return that message immediately without hitting the database.
    if (!sql.toUpperCase().startsWith("SELECT")) {
      return res.json({ answer: sql, sql: null });
    }
  } catch (err) {
